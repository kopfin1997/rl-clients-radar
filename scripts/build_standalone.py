...
.coverageGrid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px}} .coverageItem{{border:1px solid var(--line);border-radius:18px;padding:14px;background:#fff}} .coverageItem b{{display:block;font-size:15px}} .coverageItem span{{display:block;color:var(--muted);font-size:12px;margin-top:6px}}
.feed{{display:grid;gap:14px}} .news{{display:grid;grid-template-columns:240px minmax(0,1fr);gap:18px;padding:16px;border:1px solid var(--line);border-radius:24px;background:linear-gradient(180deg,#fff,#fcfdff);box-shadow:0 8px 24px rgba(18,28,45,.045)}}
...
<div class="app">
  ...
  <main class="main">
    <section>
      ...
      <div class="card" style="margin-top:16px">
        <div class="head"><h3>Client Coverage</h3><span id="coverageMeta">Loading coverage…</span></div>
        <div class="coverageGrid" id="coverageGrid"></div>
      </div>
      <div class="card" style="margin-top:16px">
...
const state = {{
  items: Array.isArray(INITIAL_HEADLINES.items) ? INITIAL_HEADLINES.items : [],
...
function coverageRows(){{
  return state.clients.map(client=>{{
    const items = state.items.filter(item=>item.client===client.name);
    const official = items.filter(item=>item.sourceType==='Official').length;
    const covers = items.filter(item=>item.imageUrl).length;
    return {{client, count:items.length, official, covers}};
  }}).sort((a,b)=>b.count-a.count || a.client.name.localeCompare(b.client.name));
}}
function renderCoverage(){{
  const rows = coverageRows();
  document.getElementById('coverageMeta').textContent = `${{rows.filter(row=>row.count>0).length}} clients currently surfaced`;
  document.getElementById('coverageGrid').innerHTML = rows.map(row=>`<div class="coverageItem"><b>${{escapeHtml(row.client.name)}}</b><span>${{escapeHtml(row.client.category)}}</span><span>${{row.count}} headline${{row.count===1?'':'s'}} · ${{row.official}} official · ${{row.covers}} with cover</span></div>`).join('');
}}
...
function renderFeed(){{
  ...
  document.getElementById('feed').innerHTML = items.length ? items.map(item=>`
    <article class="news">
      <div class="thumb">
        ${{item.imageUrl ? `<img src="${{escapeHtml(item.imageUrl)}}" alt="${{escapeHtml(item.imageAlt || item.title || item.client)}}" loading="lazy" referrerpolicy="no-referrer">` : ''}}
      ...
      <div>
        ...
        <div class="badges">
          ...
          ${{item.imageUrl ? `<span class="badge green">Real image</span>` : `<span class="badge amber">No image</span>`}}
        </div>
